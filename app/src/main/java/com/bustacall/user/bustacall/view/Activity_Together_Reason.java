...
import com.bustacall.user.bustacall.presenter.Activity_Together_Reason_Presenter;
 ...
 */
public class Activity_Together_Reason extends BaseActivity implements View.OnClickListener{
    TextView tv_user,tv_enter,tv_title;
    ListView lv_list;
    TogetherListAdapter adapter;
    Activity_Together_Reason_Presenter presenter;
    AppController app;
    int user_count;
                ...
                public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                    if (rental_list.getRental_list().get(position).getTogether().getFlag() == 1) {//빈자리 타기
                        if ((rental_list.getRental_list().get(position).getTogether().getMax_user_count() - rental_list.getRental_list().get(position).getTogether().getCurrent_user_count()) < user_count) {
                            Dialog_base dialog_base = new Dialog_base(Activity_Together_Reason.this, "인원 초과입니다.");
                            dialog_base.show();
                        } else {
